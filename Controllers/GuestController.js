...
function fetchAllGuestData(req, res, next) {
    const { errors } = validationResult(req.body);
    if (errors.length > 0) {
        logger.error("Error in fetch Guest request body");
        next(new ErrorBody(400, "Invalid values in the form"));
    } else {
        GuestService.fetchAllGuestData(req.body.guestId)
            .then((response) => {
                res.status(200);
                res.json({
                    data: response,
            ...
            .catch((error) => {
                logger.error("Failed in fetch guest: " + JSON.stringify(error));
                next(
                    new ErrorBody(
                        error.statusCode || 500,
                        error.errorMessage || "Internal Server Error"
